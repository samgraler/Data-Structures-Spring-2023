...
int main()
{
    ...
    cout << "Initial 50 Removal\n" << endl;
    UpdateChecks(tree);

    // insert last 50
	for (int x = 0; x < sub2.size(); x++) {
        hash->Insert(new int(sub2[x]));
        hash2->Insert(sub2[x]);
        tree->Insert(sub2[x]);
    }

    cout << "Secondary 50 Insertion\n" << endl;
    UpdateChecks(tree);

    // find last 50
    for (int x : sub2) {

        if (x % 9 == 0) {//finds only values where mod 9 = 0
